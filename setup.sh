...
# Opera (modern key method + skip if already installed)
sudo rm -f /etc/apt/sources.list.d/archive_uri-https_deb_opera_com_opera-stable_*.list
if ! dpkg -s opera-stable &> /dev/null; then
    if curl -fsSL https://deb.opera.com/archive.key | gpg --dearmor | sudo tee /usr/share/keyrings/opera.gpg > /dev/null; then
        ...
        sudo apt update
        safe_apt_install opera-stable
    else
        echo -e "${RED}⚠️ Opera key failed, skipping Opera${NC}"
        FAILED_APPS+=("Opera repo key download failed")
    fi
else
    SKIPPED_APPS+=("opera-stable")
fi
...
MAIN_APPS=(telegram-desktop code google-chrome-stable spotify-client)
